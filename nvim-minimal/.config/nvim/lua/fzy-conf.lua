...
map('n', "<m-p>", [[:lua fzy.execute('fd -L -t f -H -E ".git" . <C-R>=expand("%:.:h")<CR>/', fzy.sinks.edit_file)<cr>]], options)
map('n', "<leader>fc", [[:lua fzy.execute('fd -L -t f -H -E ".git" . ~/.config/', fzy.sinks.edit_file)<cr>]], options)
map('n', "<leader>fs", [[:lua fzy.execute('fd -L -t f -H -E ".git" . ~/.local/bin/', fzy.sinks.edit_file)<cr>]], options)
