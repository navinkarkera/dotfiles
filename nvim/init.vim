source ~/.config/nvim/general/settings.vim
source ~/.config/nvim/general/mappings.vim

source ~/.config/nvim/vim-plug/plugins.vim
source ~/.config/nvim/themes/gruvbox.vim
" source ~/.config/nvim/themes/oceanic.vim

source ~/.config/nvim/plug-config/asynctask.vim
source ~/.config/nvim/plug-config/fugitive-config.vim
source ~/.config/nvim/plug-config/goyo.vim
source ~/.config/nvim/plug-config/tagbar-config.vim
source ~/.config/nvim/plug-config/test.vim
source ~/.config/nvim/plug-config/vimwiki-config.vim
source ~/.config/nvim/plug-config/black-config.vim

luafile ~/.config/nvim/lua/lsp-config.lua
luafile ~/.config/nvim/lua/lsp-wrapper.lua
source ~/.config/nvim/lua/lsp-wrapper.vim
luafile ~/.config/nvim/lua/plugins/compe-config.lua
luafile ~/.config/nvim/lua/plugins/telescope-config.lua
luafile ~/.config/nvim/lua/plugins/treesitter-config.lua
luafile ~/.config/nvim/lua/plugins/lualine-config.lua
luafile ~/.config/nvim/lua/plugins/bufferline-config.lua
luafile ~/.config/nvim/lua/plugins/lua-ls.lua
