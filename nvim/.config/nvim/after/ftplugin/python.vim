...
nnoremap <silent> <buffer> <leader><F6> :lua require("harpoon.term").sendCommand(require('my-functions').count_or_one(), "python -m pytest --ds=lms.envs.tutor.test -vvs")<CR>
...
nnoremap <silent> <buffer> <leader><F7> :lua require("harpoon.term").sendCommand(require('my-functions').count_or_one(),"python -m pytest --ds=lms.envs.tutor.test -vvs <C-R>=expand('%:.')<CR>")<CR>
...
nnoremap <silent> <buffer> <leader><F8> :norm w[mw<CR> :lua require("harpoon.term").sendCommand(require('my-functions').count_or_one(), "python -m pytest --ds=lms.envs.tutor.test -vvs <C-R>=expand('%:.')<CR> -k <C-R>=expand('<cword>')<CR>")<CR>
